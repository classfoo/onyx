...
(function($) {
	var defaultOptions = {
		makeClone : false, // Drag a clone of the source, and not the
							// actual source element
		sourceClass : null, // Class to apply to source element when
							// dragging a clone of the source element
		sourceHide : false, // Specify with true that the source element
							// should hade visibility:hidden while
							// dragging a clone
		dragClass : null, // Class to apply to the element that is
							// dragged
		canDropClass : null, // Class to apply to the dragged element
								// when dropping is possible
		dropClass : null,
		isActive : true,
		container : null, // if set, dragging is limited to this
							// container
		...
		// Default is to allow dropping inside elements with css
		// stylesheet "drop"
		canDrop : function($dst) {
			return $dst.hasClass("drop")
					|| $dst.parents(".drop").size() > 0;
		},

		// Default is to move the element in the DOM and insert it into
	...
	// Status during a drag-and-drop operation. Only one such operation
	// can be in progress at any given time.
	var $sourceElement = null; // Element that user wanted to drag
	var $activeElement = null; // Element that is shown moving around
								// during drag operation
	var $destElement = null; // Element currently highlighted as
								// possible drop destination
	var dragOffsetX, dragOffsetY; // Position difference from
									// drag-point to active elements
									// left top corner
	var limits;
	...
	var methods = {
		init : function(options) {
			...
			this.bind("mousedown.dragdrop touchstart.dragdrop",
					methods.onStart);
		...
		onStart : function(event) {
			...
			if ($element) {
				...
				if (options.makeClone) {
					...
					// Elements that are cloned and dragged around are
					// added to the parent in order
				...
				if ($c) {
					...
					limits = {
						minX : offset.left,
						minY : offset.top,
						maxX : offset.left + $c.outerWidth()
								- $element.outerWidth(),
						maxY : offset.top + $c.outerHeight()
								- $element.outerHeight()
					};
				}
		...
		onMove : function(event) {
			...
			var destElement = document.elementFromPoint(posX
					- document.documentElement.scrollLeft
					- document.body.scrollLeft, posY
			...
			posY -= dragOffsetY;
			if (limits) {
				posX = Math.min(Math.max(posX, limits.minX),
						limits.maxX);
				posY = Math.min(Math.max(posY, limits.minY),
						limits.maxY);
			}
			$activeElement.css({
				left : posX,
				...
				if ($destElement == null
					...
					if (options.canDrop($possibleDestElement)) {
						if (options.dropClass) {
							if ($destElement != null)
								$destElement
										.removeClass(options.dropClass);
							$possibleDestElement
									.addClass(options.dropClass);
						}
						if (options.canDropClass) {
							$activeElement
									.addClass(options.canDropClass);
						}
	...
	$.fn.dragdrop = function(method) {
		if (methods[method]) {
			...
			$.error('Method ' + method
					+ ' does not exist on jQuery.dragdrop');
		}
	};
}));
