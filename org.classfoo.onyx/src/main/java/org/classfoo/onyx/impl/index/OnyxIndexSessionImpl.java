...
public class OnyxIndexSessionImpl implements OnyxIndexSession {
    ...
    public void mergeIndex(String index, String type, String id, Map<String, Object> object) {
        ...
        Script script = new Script(Script.DEFAULT_SCRIPT_TYPE, Script.DEFAULT_SCRIPT_LANG,
                "if(!ctx._source.objects.contains(params.object))ctx._source.objects.add(params.object)", scriptParams);
        UpdateRequest updateRequest = new UpdateRequest(index, type, id).script(script).upsert(item);
        try {
    ...
    public void upsertIndex(String index, String type, String id, Map<String, Object> object) {
        ...
        UpdateRequest updateRequest = new UpdateRequest(index, type, id).doc(object).upsert(
                indexRequest).retryOnConflict(5);
        try {
            client.update(updateRequest).get();
        }
