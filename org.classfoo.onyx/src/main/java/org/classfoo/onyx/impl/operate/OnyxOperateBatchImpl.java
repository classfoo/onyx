 ...
 *
 */
public class OnyxOperateBatchImpl extends AbstractOnyxOperate implements OnyxOperateBatch {
    ...
    public Map<String, Object> execute(OnyxStorageSession session) {
        ...
        for (OnyxOperate operate : this.operates) {
            Map<String, Object> map = ((AbstractOnyxOperate) operate).execute(session);
            results.add(map);
        }
